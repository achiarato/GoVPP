...
func createLoopback(ch api.Channel) interface_types.InterfaceIndex {
	...
	if err := ch.SendRequest(req).ReceiveReply(reply); err != nil {
		fmt.Printf("creating loopback interface: %s", err)
		return 0
	}
...
func main() {
    ...
    swIfIndex, err := CreateHostInterface(ch, "vpp-in")
    if err != nil {
    ...
    time.Sleep(1*time.Second)
    fmt.Printf("GoVPP ready to ROCK!")
//    err = GetVPPVersion(ch)
//    if err != nil {
//	fmt.Printf("Could not get VPP Version: %s\n", err)
//	os.Exit(1)
//    }
